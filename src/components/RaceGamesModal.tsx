...
/**
 * Lets the player pick which of a race's games to play. Launching a game
 * carries the `?race=` context so the play's points feed the race
 * leaderboard. Mirrors `TournamentGamesModal` — kept as a parallel component
 * because that modal hardcodes the `tournamentId`/`?tournament=` naming.
 */
const RaceGamesModal: FC<Props> = ({ raceId, raceName, games, onClose }) => {
  const navigate = useNavigate();

  const play = (key: string) => {
    onClose();
    navigate(`/games/${key}?race=${raceId}`);
  };
        ...
        <div className="p-4 overflow-y-auto space-y-2">
          {games.length === 0 ? (
            <p className="text-sm text-slate-500 py-4 text-center">
              No games configured for this race.
            </p>
          ) : (
            games.map((key) => {
